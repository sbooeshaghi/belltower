...
fn preferred_tool_choice(_provider_id: &str, request: &CompletionRequest) -> Option<String> {
    ...
    if requires_first_tool_pass(&request.messages) {
...
pub(super) fn requires_first_tool_pass(messages: &[Message]) -> bool {
    for message in messages.iter().rev() {
        if matches!(message.role, Role::Tool) && message.tool_result().is_some() {
            return false;
        }
        if matches!(message.role, Role::User) {
            let text = render_message_text_for_openai(&message.parts);
            if !text.is_empty() {
                return text_requires_tools(&text);
            }
        }
    }
...
pub(super) fn text_requires_tools(text: &str) -> bool {
